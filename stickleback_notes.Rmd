...
Get line numbers for all cut sites and subtract one to get line number of associated header/chromo:

```
zgrep -E -n -o ".{0,0}CCTGCAGG.{0,120}" bait_sbf1_seqs_to_clip.fa.gz | less -N | cut -d':' -f1 | uniq | wc -l
# make test file with all chromo lines in it (subtract one from line number)
zgrep -E -n -o ".{0,0}CCTGCAGG.{0,120}" bait_sbf1_seqs_to_clip.fa.gz | less -N | cut -d':' -f1 | awk -v s=1 '{print $1-s}' > tst.tsv
...
```
awk '{print length}' baits_sbf1_120bp_flank.txt | awk '(NR>0) && ($1 > 127 )' | less -N | wc -l
```

```
awk '{print length}' baits_sbf1_120bp_flank.txt | awk '(NR>0) && ($1 > 127 )' | shuffle
```
